...
let rec codegen_expr = function
  ...
  | Ast.Unary (op, operand) ->
    ...
    let callee =
      ...
      | Some callee -> callee
      | None -> raise (Error ("unknown unary operator: " ^ callee))
    in
  ...
  | Ast.Binary (op, lhs, rhs) ->
    begin
      match op with
      | '=' ->
        (* Special case '=' because we don't want to emit the
        ...
        let variable = try Hashtbl.find named_values name with
          | Not_found -> raise (Error ("unkown variable name: " ^ name))
        in
  ...
  | Ast.Call (callee, args) ->
    ...
    let callee =
      match lookup_function callee the_module with
      | Some callee -> callee
      | None -> raise (Error ("unkown function referenced: " ^ callee))
    in
  ...
  | Ast.If (cond, then_, else_) ->
    ...
    (* Grab the first block so that we might later add the condition branch
     * to it at then end of the function *)
     ...
     * update then_bb for the phi. We create a new name because
     * one is used for the phi node, and the other is used
    ...
    (* Return to the start block to add the condition branch *)
    position_at_end start_bb builder;
    ignore (build_cond_br cond_val then_bb else_bb builder);

    (* set a unconditional branch at the end of the 'then' block
     * and the 'else' block to the 'merge' block *)
  ...
  | Ast.For (var_name, start, end_, step, body) ->
     ...
     * where the boy of the loop mutates the variable *)
    let cur_var = build_load alloca var_name builder in
    let next_var = build_fadd cur_var step_val "nextvar" builder in
    ignore(build_store next_var alloca builder);

    (* Convert condition to a bol by comparing equal to 0.0 *)
    let zero = const_float double_type 0.0 in
